...
<body>
	...
	<?php
		if (mysql_num_rows($pubs)>=1) {
			...
			echo "<br /><h3>Não tem mais pedidos de amizade...</h3><br /><br />";
		}
	?>
	<br />
	<?php
			...
			while ($not=mysql_fetch_assoc($notificacoes)) {
				...
				if ($not['tipo']=="1") {
					echo '<br /><div class="not" id="'.$id.'">
						<a href="myprofile.php?id='.$not['post'].'">'.$nome.' gostou da sua publicação</a>
					</div>';
				}elseif($not['tipo']=="2"){
					echo '<br /><div class="not" id="'.$id.'">
						<a href="comentarios.php?id='.$not['post'].'">'.$nome.' comentou a sua publicação</a>
					</div>';
				}
			}
			echo "<br /><h3>Não tem mais notificações...</h3>";
		}else{
			echo "<br /><h3>Não tem notificações...</h3>";
		}
	?>
	<br />
	<div id="footer"><p>&copy; VM, 2018 - Todos os direitos reservados</p></div><br />
</body>
</html>